...
use binrw::{
	args, binrw, BinRead, BinReaderExt, BinResult, BinWrite, BinWriterExt, Endian, NamedArgs,
	__private::Required,
	error::CustomError,
...
impl<C, T: Debug> Debug for SizedVec<C, T> {
	fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
		...
		//     .finish()
		f.debug_tuple("SizedVec").field(&self.data.len()).finish()
	}
}
		...
		if let Some(c) = args.count {
			if self.0.len() > c {
				return Err(binrw::Error::AssertFail {
					...
					message: format!(
						"Number of bytes is greater than maximum allowed: {} > {c}",
						self.0.len()
					),
				});
			}
