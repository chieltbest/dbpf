...
impl BinRead for PropertySet {
	...
	fn read_options<R: Read + Seek>(
		...
		let overrides = (0..num_overrides)
			.map(|i| {
				Ok(Override {
					shape: get!(&format!("override{i}shape"))?,
					subset: get!(&format!("override{i}subset"))?,
					resource: Reference::read_cpf(
						&mut cpf,
						format!("override{i}resource"),
						true,
						pos,
					)?,
				})
...
impl BinWrite for PropertySet {
	...
	fn write_options<W: Write + Seek>(
		...
		cpf.entries
			.push(Item::new("numoverrides", overrides.len() as u32));
		for (
			i,
			Override {
				shape,
		...
		) in overrides.iter().cloned().enumerate()
		{
			cpf.entries
				.push(Item::new(format!("override{i}shape"), shape));
			cpf.entries
				.push(Item::new(format!("override{i}subset"), subset));
