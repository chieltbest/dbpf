...
use crate::{
	common::{BigString, SizedVec},
	internal_file::resource_collection::{
		geometric_data_container::math::{Transform, Vertex},
...
impl BlockFormat {
	...
	pub fn component_size(&self) -> usize {
		match self {
			BlockFormat::F32Scalar | BlockFormat::F32Vec2 | BlockFormat::F32Vec3 => {
				size_of::<f32>()
			}
...
impl AttributeBuffer {
	...
	pub fn elements_indexed<'a, 'b, T: Iterator<Item = &'b Reference>>(
		&'a self,
		index: T,
...
impl AttributeGroup {
	pub fn get_buffer_data<'a, 'b>(
		&'a self,
		attribute: &'b AttributeBuffer,
	...
	pub fn construct_interleaved<'a, 'b>(
		&'a self,
		gmdc: &'b GeometricDataContainer,
	) -> (
		impl Iterator<Item = &'b [u8]> + use<'a, 'b>,
		usize,
		Vec<(&'b AttributeBuffer, usize)>,
	) {
		...
		let interleaved = MultiZip::new(
			attributes
				.iter()
				.map(|(attr, _)| self.get_buffer_data(attr))
				.collect(),
		);
