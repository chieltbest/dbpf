...
use gltf_kun::{
	...
	graph::{
		gltf::{
			accessor::{iter::AccessorIter, ComponentType, Type},
			node::{Quat, Vec3},
			primitive::{Mode, Semantic},
			Accessor, GltfDocument, Node,
		},
		Graph, GraphNodeWeight,
	},
	io::format::glb::{GlbExport, GlbFormat},
};
use itertools::Itertools;
use serde_json::{Number, Value};

use crate::internal_file::resource_collection::geometric_data_container::{
	math::Mat4, AttributeBuffer, AttributeType, BlockFormat, BoundingMesh, GeometricDataContainer,
	PrimitiveType,
};
	...
	pub fn export_gltf(&self) -> Option<GlbFormat> {
			...
			.map(|element| {
				...
				let mut gltf_accessor = match element.binding.binding_type {
							...
							AccessorIter::new(
								&element
									...
									.flat_map(|e| {
										e.iter()
											.copied()
											.chain(iter::repeat(0.0f32.to_le_bytes()).flatten())
											.take(4 * size_of::<f32>())
									})
									.collect::<Vec<_>>(),
								ComponentType::F32,
						...
						Accessor::from_iter(
							...
							AccessorIter::new(
								&element
									...
									.flat_map(|e| {
										e.iter().copied().chain(
											if element.binding.binding_type
					...
					_ => {
						...
						Accessor::from_iter(
							&mut graph,
							AccessorIter::new(&element.data, component_type, element_type, false)
								.unwrap(),
						) // TODO error handling
					}
				};
		...
		fn make_bounding_mesh(
			...
			if !b_mesh.vertices.is_empty() {
				...
				let vertices_accessor = Accessor::from_iter(
					graph,
					AccessorIter::new(
						&b_mesh
							.vertices
				...
				let indices_accessor = Accessor::from_iter(
					graph,
					AccessorIter::new(
						&b_mesh
							.faces
		...
		if !self.bounding_mesh.vertices.is_empty() || !self.dynamic_bounding_mesh.is_empty() {
			let main_bounding_mesh =
				make_bounding_mesh(&doc, &mut graph, &self.bounding_mesh, "b_mesh".to_string());
			...
			bone_nodes = self
				...
				.enumerate()
				.map(|(bone_idx, b_mesh)| {
					let mut bone_bounding_mesh =
						make_bounding_mesh(&doc, &mut graph, b_mesh, format!("bone#{bone_idx}"));
			...
			let inverse_transforms_accessor = Accessor::from_iter(
				...
				AccessorIter::new(
					&self
						...
						.flat_map(|mat| mat.into_iter().flat_map(|f| f.to_le_bytes()))
						.collect::<Vec<_>>(),
					ComponentType::F32,
					Type::Mat4,
					false,
		...
		for group in self.meshes.iter() {
			...
			let blend_keys = link
				...
				.filter(|attr| attr.binding.binding_type == AttributeType::BlendKeys)
				.collect::<Vec<_>>();
			let active_morphs = blend_keys
				.iter()
				.flat_map(|keys| keys.data.iter().copied())
				.collect::<BTreeSet<_>>();
			let mut morph_to_buffer = active_morphs
				.iter()
				.copied()
				.enumerate()
				.map(|(i, m)| {
				...
				if let Some(keys) = keys {
					...
					for (buffer_key, (_, _, buffer)) in morph_to_buffer.iter_mut() {
						if keys.clone().contains(buffer_key) {
							...
							buffer.entry(b_type).or_insert_with(|| {
							...
							keys.clone()
								.zip(delta.data.chunks_exact(delta_data_chunk_size))
								.zip(
									buffer
										.get_mut(&b_type)
										.unwrap()
										.chunks_exact_mut(delta_data_chunk_size),
								)
			...
			for attr_idx in link.attributes.iter() {
				...
				let identity = match attr.binding.binding_type {
					...
					AttributeType::BlendValues1 => {
						Semantic::Extras(format!("BlendValues1_{binding_idx}"))
					}
					AttributeType::BlendValues2 => {
						Semantic::Extras(format!("BlendValues2_{binding_idx}"))
					}
					AttributeType::BoneValues => {
						Semantic::Extras(format!("BoneValues_{binding_idx}"))
					}
					AttributeType::VertexID => Semantic::Extras(format!("VertexID_{binding_idx}")),
					AttributeType::RegionMask => {
						Semantic::Extras(format!("RegionMask_{binding_idx}"))
					}
					AttributeType::DeformMask => {
						Semantic::Extras(format!("DeformMask_{binding_idx}"))
					}
					...
					AttributeType::BoneKeys => {
						...
						let bone_keys_accessor = Accessor::from_iter(
							&mut graph,
							AccessorIter::new(
								&attr
									.data
									...
									.flat_map(|i| {
										if *i == 0xff {
											...
											(group.bone_references[*i as usize].0 as u16)
												.to_le_bytes()
										}
									})
									.collect::<Vec<_>>(),
						...
						gltf_primitive.set_attribute(
							&mut graph,
							Semantic::Joints(binding_idx),
							Some(bone_keys_accessor),
						);
			...
			let morph_names = morph_to_buffer
				.iter()
				.map(|(id, (i, _, _))| {
					let name = self
			...
			for (key, (weight_idx, morph, attributes)) in morph_to_buffer {
				for (attr_type, buf) in attributes {
					...
					let accessor = Accessor::from_iter(
						&mut graph,
						AccessorIter::new(&buf, ComponentType::F32, components, false).unwrap(),
					); // TODO error handling
			...
			let _ = mesh_w.extras.insert(*Box::new(
				serde_json::value::to_raw_value(&Value::Object(
					[
						...
						(
							"opacity".to_string(),
							Value::Number(Number::from(group.opacity)),
						),
					]
					.into_iter()
					.collect(),
