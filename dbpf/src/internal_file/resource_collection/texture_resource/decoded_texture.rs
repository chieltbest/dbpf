	...
	pub fn can_shrink_dimensions(
		...
		match shrink_direction {
			ShrinkDirection::Both => {
				match (
					Self::can_shrink_dimensions(width, height, ShrinkDirection::Horizontal),
					...
					for x in 0..new_width {
						...
						for c in 0..3 {
							...
							let new_c = ((self.data[o + orig_i] as u32 * a0)
								...
								+ (self.data[o + orig_i + orig_row_offset + pixel_offset] as u32
									* a3)) / a_total;
							self.data[c + new_i] = new_c as u8;
						}
						// alpha
						if let Some(preserve_alpha) = preserve_alpha_test {
							...
							let new_c = (a0
								* (a0 * preserve_alpha + preserve_alpha_inv * preserve_alpha_inv)
								+ a1 * (a1 * preserve_alpha
									+ preserve_alpha_inv * preserve_alpha_inv)
								+ a2 * (a2 * preserve_alpha
									+ preserve_alpha_inv * preserve_alpha_inv)
								+ a3 * (a3 * preserve_alpha
									+ preserve_alpha_inv * preserve_alpha_inv))
								/ max(
									(a_total * preserve_alpha)
										+ (preserve_alpha_inv * preserve_alpha_inv * 4),
									1,
								);
