	...
	pub fn properties(&self) -> FileTypeProperties {
		...
		let (name, abbreviation, extension, embedded_filename) = match self {
			...
			TextureOverlayXML => (
				"Texture Overlay XML",
				"XTOL",
				Some("texture_overlay.xml"),
				No,
			),
			FenceArchThumbnail => (
				"Fence Arch Thumbnail",
				"THUB",
				Some("fence_arch_thumb.jpg"),
				No,
			),
			...
			SimanticsBehaviourConstants => {
				("Simantics Behaviour Constants", "BCON", None, Embedded)
			}
			...
			EdithSimanticsBehaviourLabels => {
				("Edith Simantics Behaviour Labels", "TPRP", None, Embedded)
			}
			BehaviourConstantsLabels => ("Behaviour Constants Labels", "TRCN", None, No),
			...
			NeighbourhoodObjectXML => (
				...
				"XNGB",
				Some("neighbourhood_object.xml"),
				No,
			),
			WantsTreeItemXML => (
				"Wants Tree Item XML",
				"WNTT",
				Some("wants_tree_item.xml"),
				No,
			),
			...
			AcceleratorKeyDefinitions => {
				("Accelerator Key Definitions", "KEYD", Some("keys.txt"), No)
			}
