...
impl FilteredConflictList {
	pub fn new(storage: &Option<&dyn Storage>) -> Self {
		...
		if let Some(storage) = storage {
			if let Some(known_conflicts_str) = storage.get_string("known_conflicts") {
	...
	pub fn save(&mut self, storage: &mut dyn Storage) {
		...
		for t in FilteredConflictList::filter_types() {
			storage.set_string(
				format!("check_{}", t.properties().abbreviation).as_str(),
	...
	pub fn is_known(&self, conflict: &TGIConflict) -> bool {
		self.known_conflicts
			.iter()
			.any(|known| known.is_known(conflict))
	}
	...
	pub fn get_type_visibility(
		&self,
		internal_type: &KnownDBPFFileType,
	) -> ConflictTypeFilterWarning {
	...
	fn filter_conflict(&mut self, conflict: TGIConflict) {
		...
		for tgi in &conflict.tgis {
			if let DBPFFileType::Known(t) = tgi.type_id {
				if self.get_check_enabled(&t) && (self.show_known || !known) {
		...
		if is_shown {
			...
			for known_t in all_conflict_tgis {
				self.found_conflict_visible
					.entry(known_t)
					.or_insert(ConflictTypeFilterWarning::FoundVisible);
			}
		} else if !known {
			...
			for known_t in all_conflict_tgis {
				self.found_conflict_visible
					.insert(known_t, ConflictTypeFilterWarning::NotVisible);
			}
		}
	}
