...
use eframe::{
	egui,
	egui::{
		containers, Color32, Context, DragValue, Label, ProgressBar, RichText, Sense, Style,
		TextEdit, Ui, Visuals, Window,
	},
	App, Frame, Storage,
};
...
impl DBPFApp {
	...
	fn known_conflict_menu(&mut self, ctx: &Context, ui: &mut Ui) {
			...
			.show(ctx, |ui| {
				...
				if !known_conflicts.is_empty() {
					...
					egui_extras::TableBuilder::new(ui)
						...
						.body(|body| {
							body.rows(14.0, known_conflicts.len(), |mut row| {
								let i = row.index();
		...
		ui.button("Known conflicts").clicked().then(|| {
	...
	fn conflict_menu(&mut self, path: &Path, conflict: &TGIConflict, ui: &mut Ui) {
		ui.button("Add to known conflicts").clicked().then(|| {
			self.conflict_list.add_known(KnownConflict(
	...
	fn show_path_cell(&mut self, conflict: &TGIConflict, path: &Path, ui: &mut Ui) -> bool {
		...
		frame.show(ui, |ui| {
			ui.horizontal_centered(|ui| {
				let lbl = ui
					.add(Label::new(text).extend().sense(Sense::click()))
		...
		egui_extras::TableBuilder::new(ui)
			...
			.body(|body| {
				...
				body.rows(14.0, filtered.len(), |mut row| {
					...
					row.col(|ui| {
						if self.show_path_cell(conflict, &conflict.original, ui) {
							highlight = Some(conflict.clone());
						}
					});
					row.col(|ui| {
						if self.show_path_cell(conflict, &conflict.new, ui) {
							highlight = Some(conflict.clone());
						}
					});
				});
...
impl App for DBPFApp {
		...
		egui::CentralPanel::default().show(ctx, |ui| {
			ui.vertical(|ui| {
				...
				if let Some((ref path, progress, total)) =
					*self.find_conflict_progress.lock().unwrap()
				{
					ui.add(
						ProgressBar::new(progress as f32 / total as f32).text(
							Self::strip_prefix(&self.scan_ran_with_folders, path)
								.unwrap_or(path)
								.display()
								.to_string(),
	...
	fn save(&mut self, storage: &mut dyn Storage) {
		...
		storage.set_string(
			"open_known_conflict_gui",
			self.open_known_conflict_gui.to_string(),
		);
