...
pub struct GMDCEditorStateData {
    ...
    subsets: Vec<(glow::NativeVertexArray, glow::NativeBuffer, glow::NativeBuffer, usize, usize, bool)>,
        ...
        if let Some(gl) = gl_context {
            ...
            unsafe {
                let shader_sources = [
                    ...
                    (glow::FRAGMENT_SHADER, OFFSCREEN_RENDER_FRAGMENT_SHADER_SOURCE),
                ];
                ...
                let Some(subsets) = std::iter::once(&self.bounding_mesh)
                    ...
                    .map(|subset| {
                        ...
                        gl.buffer_data_u8_slice(glow::ARRAY_BUFFER, &subset_vertex_data, glow::STATIC_DRAW);
                        let position_location = gl.get_attrib_location(main_program, "in_position").unwrap(); // TODO unwrap
                        gl.vertex_attrib_pointer_f32(position_location,
                                                     3,
                                                     glow::FLOAT,
                                                     false,
                                                     3 * 4,
                        ...
                        gl.buffer_data_u8_slice(glow::ELEMENT_ARRAY_BUFFER, &subset_index_data, glow::STATIC_DRAW);
                ...
                let attribute_objects = self.attribute_groups.iter()
                    .map(|group| {
                        let vao = gl.create_vertex_array()
                            .inspect_err(|err| error!(?err))
                            .ok()?;
                        ...
                        for (attr, offset) in attributes {
                            ...
                            let (attr_binding_name, attr_type) = match attr.binding.binding_type {
                                ...
                                AttributeType::BlendValues2 => ("in_target_indices", glow::UNSIGNED_BYTE),
                            ...
                            if let Some(location) = location {
                                ...
                                gl.bind_buffer(glow::ARRAY_BUFFER, Some(vbo));
                                gl.vertex_attrib_pointer_f32(attr_binding,
                ...
                let meshes = self.meshes.iter()
                    .map(|mesh| {
                        attribute_objects[mesh.attribute_group_index as usize]
                            .and_then(|vao| {
                                let indices = gl.create_buffer()
                                    .inspect_err(|err| error!(?err))
                                    .ok()?;

                                let mesh_index_data: Vec<u8> = mesh.indices.iter()
                                    .flat_map(|f| f.0.to_le_bytes())
                                    .collect();
                                ...
                                gl.buffer_data_u8_slice(glow::ELEMENT_ARRAY_BUFFER, &mesh_index_data, glow::STATIC_DRAW);
                ...
                let Ok(or_vao) = gl.create_vertex_array()
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ui.horizontal_wrapped(|ui| {
                        ...
                        ui.group(|ui| {
                            ui.vertical(|ui| {
                                for (mesh, gl_mesh) in self.meshes.iter_mut()
                                    .zip(gl_state.meshes.iter_mut()) {
                                    ui.horizontal(|ui| {
                                        ui.add(egui::Checkbox::without_text(&mut gl_mesh.visible));

                                        mesh.name.show_editor(&mut 100.0, ui)
                                            .on_hover_ui(|ui| {
                                                ui.label(format!("{} {:?}",
                                                                 mesh.poly_count(),
                                                                 mesh.primitive_type,));

                                                let bones = mesh.bone_references.iter()
                                                    .map(|r| r.0)
                                                    .collect::<Vec<_>>();
                                                ui.label(format!("Bones: {:?}", bones));

                                                ui.label("Attributes:");
                                                let attribute_group = &self.attribute_groups[mesh.attribute_group_index as usize];
                                                for attr_idx in attribute_group.attributes.iter() {
                                                    let attr = &self.attribute_buffers[attr_idx.0 as usize];
                                                    ui.label(format!("{:?}#{}", attr.binding.binding_type, attr.binding.binding_slot));
                                                }
                                            });
                        ...
                        if !self.blend_group_bindings.is_empty() {
                            ui.group(|ui| {
                                ui.vertical(|ui| {
                                    for (name, value) in self.blend_group_bindings.iter_mut()
                                        .zip(&mut gl_state.blend_values) {
                                        ui.horizontal(|ui| {
                                            ui.add(egui::Slider::new(value, 0.0..=1.0)
                                                .clamping(SliderClamping::Never));
                                            name.blend_group.show_editor(&mut 70.0, ui)
                                                .on_hover_text("Blend group");
                                            name.element.show_editor(&mut 70.0, ui)
            ...
            egui::Frame::canvas(ui.style())
                .show(ui, |ui| {
                    ...
                    if ui.input(|i| i.pointer.button_down(PointerButton::Secondary)) {
                        gl_state.camera_position += inverse_orientation * Vertex {
                            x: drag_delta.x,
                            y: -drag_delta.y,
                            z: 0.0,
                        };
                    ...
                    if response.hovered() {
                        let scroll_delta = ui.input(|i| i.smooth_scroll_delta) / rect.height() * 2.0;
                        gl_state.camera_position += inverse_orientation * Vertex {
                            x: scroll_delta.x,
                            y: 0.0,
                            z: -scroll_delta.y,
                    ...
                    let cb = egui_glow::CallbackFn::new(move |info, painter| {
                        ...
                        unsafe {
                            ...
                            gl.framebuffer_texture_2d(glow::FRAMEBUFFER, glow::COLOR_ATTACHMENT0, glow::TEXTURE_2D, Some(ctex), 0);

                            let fbstatus = gl.check_framebuffer_status(glow::FRAMEBUFFER);
                            if fbstatus != glow::FRAMEBUFFER_COMPLETE {
                                error!("framebuffer is incomplete, OpenGL error code: {}", fbstatus);
                            }
                            ...
                            let model_mat = Mat4::projection(0.1, 1.0 / info.viewport.aspect_ratio())
                                * Mat4::translation(Vertex { x: 0.0, y: 0.0, z: state.camera_distance })
                            ...
                            gl.uniform_1_f32_slice(
                                gl.get_uniform_location(state.program, "blend_values").as_ref(),
                                &state.blend_values);
                            ...
                            gl.uniform_1_i32(
                                gl.get_uniform_location(state.program, "display_mode").as_ref(),
                                display_mode);
                            ...
                            for mesh in state.meshes.iter()
                                ...
                                gl.vertex_attrib_4_f32(
                                    gl.get_attrib_location(state.program, "in_blend_weights").unwrap(),
                                    1.0, 1.0, 1.0, 1.0,
                                );

                                gl.uniform_matrix_4_f32_slice(
                                    gl.get_uniform_location(state.program, "view_matrix").as_ref(),
                                    false,
                                ...
                                gl.draw_elements(mesh.primitive_type,
                            ...
                            gl.uniform_1_i32(gl.get_uniform_location(state.offscreen_render_program, "t").as_ref(), 0);
