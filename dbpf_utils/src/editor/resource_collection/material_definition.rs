...
impl Editor for Property {
    ...
    fn show_editor(&mut self, _state: &mut Self::EditorState, ui: &mut Ui, gl: &Option<Arc<glow::Context>>) -> Response {
        self.name.show_editor(&mut 300.0, ui, gl)
            | self.value.show_editor(&mut 300.0, ui, gl)
    }
}
...
impl Editor for MaterialDefinition {
    ...
    fn show_editor(&mut self, _state: &mut Self::EditorState, ui: &mut Ui, gl: &Option<Arc<glow::Context>>) -> Response {
        ...
        ui.horizontal(|ui| {
            ...
            res |= self.material_type.show_editor(&mut 500.0, ui, gl);
        });
        ui.horizontal(|ui| {
            ui.label("Material Description");
            res |= self.material_description.show_editor(&mut 500.0, ui, gl);
        });
