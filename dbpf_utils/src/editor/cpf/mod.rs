...
impl Editor for Item {
	...
	fn show_editor(&mut self, _state: &mut Self::EditorState, ui: &mut Ui) -> Response {
		let mut res = ComboBox::from_id_salt("data_type")
			...
			.show_ui(ui, |ui| {
				if ui.button("UInt").clicked() {
					self.data = Data::UInt(0);
				}
				if ui.button("Int").clicked() {
					self.data = Data::Int(0);
				}
				if ui.button("Float").clicked() {
					self.data = Data::Float(0.0);
				}
				if ui.button("Bool").clicked() {
					self.data = Data::Bool(false);
				}
				if ui.button("String").clicked() {
					self.data = Data::String("".into());
				}
			})
			.response;
	...
	fn show_editor(&mut self, _state: &mut Self::EditorState, ui: &mut Ui) -> Response {
		let ires = ui.horizontal_wrapped(|ui| {
			...
			match &mut self.version {
				CPFVersion::CPF(version) => res |= ui.add(DragValue::new(version)),
