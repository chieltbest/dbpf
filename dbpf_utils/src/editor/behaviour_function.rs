...
use dbpf::internal_file::behaviour::behaviour_function::{
	BehaviourFunction, Goto, Instruction, Signature,
};
use eframe::{
	egui::{Color32, ComboBox, Context, DragValue, Grid, Pos2, Response, Ui},
	glow,
};
use egui_snarl::{
	ui::{PinInfo, PinPlacement, SnarlPin, SnarlStyle, SnarlViewer, WireLayer, WireStyle},
...
impl EnumEditor for Goto {
	...
	fn full_name(&self) -> String {
		match self {
			Goto::Error | Goto::True | Goto::False => format!("{self:?}"),
		...
		res | ui
			.horizontal(|ui| {
				...
				let res = self
					.operands
					.iter_mut()
					.map(|u| ui.add(DragValue::new(u).hexadecimal(2, false, false)))
					.reduce(|r1, r2| r1 | r2)
					.unwrap();
				ui.label(")");
				res
	...
	fn new_editor(
		...
		while let Some(first) = unopened.pop_first() {
			...
			while let Some((id, x, y)) = open_list.pop_back() {
				...
				let targets =
					[instruction.true_target, instruction.false_target].map(|t| match t {
						Goto::Instr(i) => unopened.remove(&(i as usize)).then_some(i as usize),
						_ => None,
					});
				match targets {
					[Some(t), Some(f)] => {
		...
		for group in sorted_instr_groups.into_iter() {
			let group_min = group.iter().map(|(_, x, _)| x).min().unwrap();
		...
		for (instr, pos) in self.instructions.iter().zip(node_positons) {
