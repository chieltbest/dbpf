...
use eframe::egui;
use eframe::egui::{CollapsingResponse, ComboBox, Response, SliderClamping, Ui, WidgetText};
use serde::{Deserialize, Serialize};

const SDSC_SHARED_PERSIST_KEY: &str = "SDSC shared state";
...
struct SimDescriptionSharedState {
	character_open: bool,
	current_state_open: bool,
	aspiration_open: bool,
	personality_open: bool,
	skills_open: bool,
	decay_open: bool,
	interests_open: bool,
	school_work_open: bool,
	relations_open: bool,
	traits_open: bool,
	university_open: bool,
	nightlife_open: bool,
	business_open: bool,
	pets_open: bool,
	castaway_open: bool,
	bon_voyage_open: bool,
	free_time_open: bool,
	apartment_life_open: bool,
	internal_open: bool,
	internal_interaction_open: bool,
	internal_routing_open: bool,
	internal_loco_open: bool,
	internal_look_at_open: bool,
}
...
impl Editor for SimDescription {
	...
	fn show_editor(&mut self, _state: &mut Self::EditorState, ui: &mut Ui) -> Response {
		let mut shared_state: SimDescriptionSharedState = ui.memory_mut(|memory| {
			*memory
				.data
				.get_persisted_mut_or_default(SDSC_SHARED_PERSIST_KEY.into())
		});
		...
		collapsing(ui, "Character", &mut shared_state.character_open, |ui| {
			egui::Grid::new("SimDescription editor character").show(ui, |ui| {
				...
				ui.end_row();
				ui.label("");
				ui.add(egui::Slider::new(preference_gender_female, -1000..=1000).text("female"));
				ui.end_row();
				...
				ui.add(egui::Slider::new(skill_music, -100..=100).clamping(SliderClamping::Edits));
				ui.end_row();
		...
		collapsing(
			ui,
			"Current State",
			&mut shared_state.current_state_open,
			|ui| {
		...
		collapsing(ui, "Aspiration", &mut shared_state.aspiration_open, |ui| {
		...
		collapsing(
			ui,
			"Personality",
			&mut shared_state.personality_open,
			|ui| {
		...
		collapsing(ui, "Skills", &mut shared_state.skills_open, |ui| {
		...
		collapsing(ui, "Decay", &mut shared_state.decay_open, |ui| {
		...
		collapsing(ui, "Interests", &mut shared_state.interests_open, |ui| {
		...
		collapsing(
			ui,
			"School/Work",
			&mut shared_state.school_work_open,
			|ui| {
		...
		collapsing(ui, "Relations", &mut shared_state.relations_open, |ui| {
		...
		collapsing(ui, "Traits", &mut shared_state.traits_open, |ui| {
		...
		collapsing(ui, "University", &mut shared_state.university_open, |ui| {
		...
		collapsing(ui, "Nightlife", &mut shared_state.nightlife_open, |ui| {
		...
		collapsing(ui, "Business", &mut shared_state.business_open, |ui| {
		...
		collapsing(ui, "Pets", &mut shared_state.pets_open, |ui| {
		...
		collapsing(ui, "Castaway", &mut shared_state.castaway_open, |ui| {
		...
		collapsing(ui, "Bon Voyage", &mut shared_state.bon_voyage_open, |ui| {
		...
		collapsing(ui, "Free Time", &mut shared_state.free_time_open, |ui| {
			ui.add_enabled_ui(*version >= Version::FreeTime, |ui| {
				...
				egui::Grid::new("SimDescription editor free time lta").show(ui, |ui| {
					...
					drag_decimal_fn("lifetime aspiration points", lifetime_aspiration_points, ui);
					drag_decimal_fn(
		...
		collapsing(
			ui,
			"Apartment Life",
			&mut shared_state.apartment_life_open,
			|ui| {
		...
		let res = collapsing(ui, "Internal", &mut shared_state.internal_open, |ui| {
			collapsing(
				ui,
				"Interaction",
				&mut shared_state.internal_interaction_open,
				|ui| {
			...
			collapsing(
				ui,
				"Routing",
				&mut shared_state.internal_routing_open,
				|ui| {
			...
			collapsing(ui, "Loco", &mut shared_state.internal_loco_open, |ui| {
			...
			collapsing(
				ui,
				"Look At",
				&mut shared_state.internal_look_at_open,
				|ui| {
		...
		ui.memory_mut(|memory| {
			memory
				.data
				.insert_persisted(SDSC_SHARED_PERSIST_KEY.into(), shared_state)
		});
